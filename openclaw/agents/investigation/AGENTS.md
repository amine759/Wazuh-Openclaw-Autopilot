...
**After completing investigation, you MUST invoke `exec` with `curl` to advance the pipeline.** If you skip this step, the pipeline stalls and the Response Planner Agent is never triggered.

Invoke `exec` with `curl` with the following URL (replace `{case_id}` with the actual case ID from the webhook message):

    exec(command="curl -s 'http://127.0.0.1:9090/api/agent-action/update-case?case_id={case_id}&status=investigated&token=<AUTOPILOT_MCP_AUTH>'")

To attach your investigation findings, add a URL-encoded JSON `data` parameter:

    exec(command="curl -s 'http://127.0.0.1:9090/api/agent-action/update-case?case_id={case_id}&status=investigated&data=%7B%22recommended_response%22%3A%5B%22Block+attacker+IP%22%5D%7D&token=<AUTOPILOT_MCP_AUTH>'")
